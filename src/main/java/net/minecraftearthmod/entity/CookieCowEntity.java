...
public class CookieCowEntity extends Animal  {


	public CookieCowEntity(PlayMessages.SpawnEntity packet, Level world) {
    	this(MinecraftEarthModModEntities.COOKIE_COW.get(), world);
    }

	public CookieCowEntity(EntityType<CookieCowEntity> type, Level world) {
    	super(type, world);
		maxUpStep = 0.6f;
		xpReward = 3;
		setNoAi(false);
...
;
        ...
        @Override public AgeableMob getBreedOffspring(ServerLevel serverWorld, AgeableMob ageable) {
			CookieCowEntity retval = MinecraftEarthModModEntities.COOKIE_COW.get().create(serverWorld);
	...
	public static void init() {
			SpawnPlacements.register(MinecraftEarthModModEntities.COOKIE_COW.get(),
