...
public class JollyLlamaEntity extends Animal  {


	public JollyLlamaEntity(PlayMessages.SpawnEntity packet, Level world) {
    	this(MinecraftEarthModModEntities.JOLLY_LLAMA.get(), world);
    }

	public JollyLlamaEntity(EntityType<JollyLlamaEntity> type, Level world) {
    	super(type, world);
		maxUpStep = 0.6f;
		xpReward = 3;
		setNoAi(false);
...
);this.goalSelector.addGoal(3, new RandomStrollGoal(this, 1));this.goalSelector.addGoal(4, new RandomLookAroundGoal(this));this.goalSelector.addGoal(5, new FloatGoal(this));
	...
	@Override public InteractionResult mobInteract(Player sourceentity, InteractionHand hand) {
        ...
        @Override public AgeableMob getBreedOffspring(ServerLevel serverWorld, AgeableMob ageable) {
			JollyLlamaEntity retval = MinecraftEarthModModEntities.JOLLY_LLAMA.get().create(serverWorld);
	...
	public static void init() {
			SpawnPlacements.register(MinecraftEarthModModEntities.JOLLY_LLAMA.get(),
