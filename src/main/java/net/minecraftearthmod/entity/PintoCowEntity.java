...
public class PintoCowEntity extends Animal  {


	public PintoCowEntity(PlayMessages.SpawnEntity packet, Level world) {
    	this(MinecraftEarthModModEntities.PINTO_COW.get(), world);
    }

	public PintoCowEntity(EntityType<PintoCowEntity> type, Level world) {
    	super(type, world);
		maxUpStep = 0.6f;
		xpReward = 3;
		setNoAi(false);
...
;
        ...
        @Override public AgeableMob getBreedOffspring(ServerLevel serverWorld, AgeableMob ageable) {
			PintoCowEntity retval = MinecraftEarthModModEntities.PINTO_COW.get().create(serverWorld);
	...
	public static void init() {
			SpawnPlacements.register(MinecraftEarthModModEntities.PINTO_COW.get(),
