		...
		if (entity instanceof HornedSheepEntity) {
			if (entity.getPersistentData().getDouble("regrow") >= 6000) {
				...
				if (world instanceof Level _level) {
					if (!_level.isClientSide()) {
						_level.playSound(null, new BlockPos(x, y, z),
								ForgeRegistries.SOUND_EVENTS.getValue(new ResourceLocation("entity.sheep.ambient")), SoundSource.NEUTRAL, 1, 1);
					} else {
						_level.playLocalSound(x, y, z, ForgeRegistries.SOUND_EVENTS.getValue(new ResourceLocation("entity.sheep.ambient")),
								SoundSource.NEUTRAL, 1, 1, false);
					}
				}
			}
		}
		if (entity instanceof RainbowSheepEntity) {
			if (entity.getPersistentData().getDouble("regrow") >= 6000) {
				if ((sourceentity instanceof LivingEntity _livEnt ? _livEnt.getMainHandItem() : ItemStack.EMPTY).getItem() == Items.SHEARS) {
					if (!(sourceentity instanceof Player _plr ? _plr.getAbilities().instabuild : false)) {
						{
